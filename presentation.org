...
*Result*: 17.5x speedup on 512×512 matrices (11.36s → 0.65s)
...
*Problem*: Row-major matrix B accessed by columns → cache disaster
...
- *L2*: 1.5 MB per core → crossover at ~384×384
...
*Our bottlenecks*:
1. *Buffer copying overhead* - Load matrix → buffer → dotprod (3x penalty)
