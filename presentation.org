...
*The bottleneck*: CPUs perform ~10⁹ operations/second, but naive matrix multiplication is slow
...
** Problem:
Row-major matrix B accessed by columns → cache disaster
** Solution:
...
#+end_example
  ...
  --------|--------|-----------|--------------|------------------
  256×256 | 1.27   | 2.49      | 1.83 (b=32)  | 3.46
  512×512 | 0.49   | 2.50      | 1.86 (b=32)  | 3.66
  768×768 | 0.60   | 2.56      | 1.92 (b=32)  | 3.60
...
* Hardware Performance Counters: The Smoking Gun (512×512, 20 iterations)

| Metric                | Naive      | Blocked    | SIMD       | nalgebra   |
|-----------------------+------------+------------+------------+------------|
| *Time*                | 11.56s     | 3.26s      | 0.66s      | 0.22s      |
| *Instructions*        | 32.3B      | 30.4B      | 4.7B       | 1.5B       |
| *L1 cache misses*     | 2.54B      | 2.54B      | 38.5M      | 7.5M       |
| *L1 miss rate*        | 48%        | 46%        | 2.0%       | 2.8%       |
| *LLC loads*           | 2.26B      | 1.4M       | 2.2M       | 0.77M      |
|-----------------------+------------+------------+------------+------------|
| *Backend Bound*       | 76.8%      | 12.9%      | 21.6%      | 40.6%      |
| *Memory Bound*        | 41.7%      | 1.6%       | 7.8%       | 4.3%       |
| *Retiring (useful)*   | 20.5%      | 65.1%      | 62.7%      | 57.0%      |

*Key insights*:
- Naive spends 41.7% of time waiting on memory - *cache catastrophe*!
- Blocked fixes LLC (L3) but not L1 - still 46% L1 miss rate
- SIMD + column-major: *66× fewer L1 misses* (2.5B → 38M)
- Blocked does *3× more useful work* (65% retiring vs 20%)
- nalgebra executes *7× fewer instructions* than SIMD - algorithmic superiority
...
for i in ii..i_end {
    ...
    for j in jj..j_end {
        fill_col_buf(j);       // ← PROBLEM: Loaded 4,096 times!
...
*Result*: 262,144 redundant buffer fills per 64×64 block
...
*Impact*:
- Time: 1.49s → 0.65s (*2.3x speedup*)
- Instructions: 13.6B → 4.8B (*2.8x fewer*)
- L1 miss rate: 10.1% → 2.2% (*4.6x better*)
...
*Result*: 1.49s → 1.49s (*0% improvement!*)
...
512×512 matrices, 20 iterations:

| Implementation | Time    | L1 Miss Rate | Speedup |
|----------------+---------+--------------+---------|
| Naive          | 11.36s  | 48.5% 💥     | 1.0x    |
| Blocked        | 3.04s   | 45.8%        | 3.7x    |
| SIMD           | 1.49s   | 8.5%         | 7.7x    |
| *Optimized*    | *0.65s* | *2.2%* ✅    | *17.5x* |
...
2. *Cache hierarchy is multi-level* - Column-major helps L1
3. *Measure with hardware counters* - L1 miss rates reveal true bottlenecks

* Next Steps
** What Works
- ✅ Cache blocking (3.7x improvement)
- ✅ Column-major layout (reduces L1 misses 5x)
- ✅ SIMD vectorization (parallelism)
- ✅ Profiling-driven optimization (found 49% overhead)

** What Doesn't
- ❌ FMA instructions (0% improvement when algorithm bottlenecked)
- ❌ More blocking alone (can't fix buffer copy overhead)

** To Match Production Libraries
- Register tiling (keep 4×4 sub-blocks in registers)
